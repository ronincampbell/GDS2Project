...
@export var can_give_fireball: bool = true
@export var can_give_shield: bool = true
...
func _on_body_entered(body: Node) -> void:
	if !active:
		return
	var caster := body.get_node_or_null("SpellCaster")
	if caster == null:
		return
	var pool: Array = []
	if can_give_fireball:
		pool.append(SpellCaster.SpellID.FIREBALL)
	if can_give_shield:
		pool.append(SpellCaster.SpellID.SHIELD)
	if pool.is_empty():
		return
	var spell: Variant = pool[randi() % pool.size()]
	if caster.give_spell(spell):
		_set_active(false)
